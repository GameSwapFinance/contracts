...
// MasterChef is the master of Stonk.
// Note that it's ownable and the owner wields tremendous power. 
contract StonkChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    struct PoolInfo {
        ...
        uint256 accStonkPerShare;   // Accumulated EGGs per share, times 1e12. See below.
    ...
    // Bonus muliplier for early Stonk makers.
    ...
    // The block number when Stonk mining starts.
    ...
    function add(uint256 _allocPoint, IERC20 _lpToken, uint16 _depositFeeBP, uint256 _harvestInterval, bool _withUpdate) public onlyOwner { // TODO harvest interval
    ...
    // Update the given pool's Stonk allocation point and deposit fee. Can only be called by the owner.
    function set(uint256 _pid, uint256 _allocPoint, uint16 _depositFeeBP, uint256 _harvestInterval, bool _withUpdate) public onlyOwner { //todo harvest interval
    ...
    function pendingStonk(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            accStonkPerShare = accStonkPerShare.add(stonkReward.mul(1e12).div(lpSupply));
        }
        uint256 pending = user.amount.mul(accStonkPerShare).div(1e12).sub(user.rewardDebt);
    ...
    function payOrLockupPendingToken(uint256 _pid) internal {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeStonkTransfer(msg.sender, totalRewards);
            }
        } else if (pending > 0) {
    ...
    function updateEmissionRate(uint256 _stonkPerBlock) public onlyOwner {
        massUpdatePools();
        stonkPerBlock = _stonkPerBlock;
    }

    // Update the start block.  Can only update before start of farming.
    function updateStartBlock(uint256 _startBlock) public onlyOwner {
        require(_startBlock > block.number, "can't modify block if farming has already started");
        startBlock = _startBlock;
    }
}
